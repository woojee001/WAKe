...
class WakeGuiApp():
    ...
    def _checkSessionId(self, cookie, request, no_standard_checks=False, check_duplicate=False, static_username='',
                        set_duplicate=False):
        '''
        ...
        if not no_standard_checks:
            ...
            if 'expiration_time' in self._user_sessions[cookie.value[0:32]]:
                if time.time() > self._user_sessions[cookie.value[0:32]]['expiration_time'] + self._SESSION_DURATION\
                        and self._user_sessions[cookie.value[0:32]]['expiration_time'] > 0:
                    self._user_sessions_lock.acquire()
    ...
    def _checkSession(self, request, no_standard_checks=False, check_duplicate=False, static_username='',
                      set_duplicate=False):
        '''
    ...
    def getPageHeader(self):
        ...
        if session_status['authenticated']:
            try:
                html = u''.join(open(join(self._BASE_DIR, 'resources', 'html',
                                          'authenticated_header.html')).readlines())
            except:
                html = 'NO HEADER'
            return '{{"success": true, "html": {0:s}}}'.format(json.dumps(
                html.replace('~~USERNAME~~', self._user_sessions[wake_session_id]['username'])))
    ...
    def connect(self, username='', password=''):
        ...
        if connected:
            ...
            if session_status['duplicate']:
                ...
                self._user_sessions_lock.release()
                return '{"success": true, "duplicate": true, "msg": "An older session with the username you specified'\
                        ' already exists. Do you wish to overwritte it ?"}'
    ...
    def logout(self):
        ...
        raise cherrypy._cperror.HTTPRedirect('/', 302)
...
class WakeGui(multiprocessing.Process):
    ...
    def run(self):
        ...
        appconf = {
            ...
            '/favicon.ico': {'tools.staticfile.on': True, 'tools.staticfile.filename': join(self._BASE_DIR,
                                                                                            'favicon.ico')},
            '/login.js': {'tools.staticfile.on': True, 'tools.staticfile.filename': join(self._BASE_DIR, 'login.js')},
            '/change_pwd.js': {'tools.staticfile.on': True, 'tools.staticfile.filename': join(self._BASE_DIR,
                                                                                              'change_pwd.js')},
